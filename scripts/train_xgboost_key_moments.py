# scripts/train_xgboost_key_moments.py - Add time x quality "key moment" features
"""Adds the joint time/eval interaction features (critical_loss,
critical_time_ratio, etc. - see src/time_quality_features.py) on top of the
full-stack feature set (basic + move-replay + engine + time). Compares
against xgb_full_stack_v9 (MAE 213.2) on the same 114k-game subset.

Usage: python scripts/train_xgboost_key_moments.py
"""
import json
import sys
import time
...
KEY_MOMENT_FEATURE_COLUMNS = [
    "white_critical_time_ratio", "black_critical_time_ratio",
    "white_critical_loss", "black_critical_loss",
    "white_quiet_loss", "black_quiet_loss",
    "white_fast_critical_errors", "black_fast_critical_errors",
    "white_slow_critical_success", "black_slow_critical_success",
    "white_time_loss_corr", "black_time_loss_corr",
    "white_critical_moments", "black_critical_moments",
]
...
def main():
    ...
    print(f"  full subset: {len(base_df):,} games")
    ...
    X_key = merged_key[KEY_MOMENT_FEATURE_COLUMNS].reset_index(drop=True)
    X = pd.concat([X_basic.reset_index(drop=True), X_engine, X_time, X_key], axis=1)
    ...
    print("\nTraining XGBRegressor (+ key-moment time x quality features)...")
    t0 = time.time()
    model = XGBRegressor(
    ...
    print(f"R2:   {r2:.3f}")
    print(f"\nvs. xgb_full_stack_v9: MAE {BENCHMARK_MAE} -> {mae:.1f}  ({mae-BENCHMARK_MAE:+.1f})")
    ...
    model_path = MODELS_DIR / "xgb_key_moments_v10.joblib"
    joblib.dump({"model": model, "feature_columns": list(X.columns)}, model_path)
    print(f"\nSaved model: {model_path}")

    metrics = {
        "model": "XGBRegressor_key_moments_v10", "n": len(X),
    ...
    (MODELS_DIR / "xgb_key_moments_v10_metrics.json").write_text(json.dumps(metrics, indent=2))
    print(f"Saved metrics: {MODELS_DIR / 'xgb_key_moments_v10_metrics.json'}")
